...
import environment from "../../../assets/environments.png";
...
<YouTube posterQuality="max" id="pRLoGyuu_BE" />
...
Se till att godkänna Actions och att sätta GitHub Actions som sätt att publicera GitHub Pages webbplatsen. Se även till att lägga `kmom10` branchen som en Deployment Branch, så inte `kmom0*` men `kmom10` eller `kmom*` i bilden nedan.

<Image src={environment} alt="Add deployment branch in GitHub" />
...
Din webapp ska baseras på de tekniker vi har använt under kursens gång. Du ska använda vite för att bygga och driftsätta appen med hjälp GitHub Actions. Din app ska vara en PWA med en egen Logga.

__Arkitektur__: Beskriv i ett textstycke, som en del av din inlämning, vilka val av teknik du har gjort för din app. Berätta hur du har organiserad din kod så att en kollega snabbt kan sätta sig in i din app. Beskriv vilken data du har valt att göra en cache av.
...
Använd positionsdata som finns för stationerna för att visa upp förseningar i tågtrafiken på en karta med hjälp av de tekniker vi använde i kursmoment 4. Använd Web API:t GeoLocation för att visa upp användarens position på kartan.
...
Gör en Pull Request från kmom10 branchen mot din egna forks main-branch. Som en del av din Pull Request skriv följande reflektionstext:

...
Du har gjort en fork av webapp-lager repot och gjort övningarna [Typografi i mobila enheter](/kunskap/typografi-i-mobila-enheter) och [Knappar för mobilen](/kunskap/knappar-for-mobilen).
...
I `index.html` tar vi bort `<product-list>` komponenten och har bara `<router-outlet></router-outlet>`.
...
Se även till att importera och definiera ett customElement i `main.js`, min fil ser ut enligt nedan vid detta läget.
